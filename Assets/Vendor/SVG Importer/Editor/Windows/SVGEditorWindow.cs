...
namespace SVGImporter
{
	...
	internal class SVGEditorWindow : EditorWindow
	{
        ...
        void OnGUI () 
        {
            ...
            bool selectedObject = svgAsset != null && Selection.objects.Length == 1;
        ...
        Camera editorCamera
        {
            get {
                if(_editorCamera == null)
                {
                    ...
                    _editorCamera.targetTexture = _editorPreview;
        ...
        void CreateRenderTexture()
        {
            ...
            {
                _editorPreview.DiscardContents();
            ...
            _editorPreview = new RenderTexture(_previewResolution, Mathf.CeilToInt(_previewResolution / aspect), 24, RenderTextureFormat.Default, RenderTextureReadWrite.Default);
            _editorPreview.antiAliasing = 4;
            _editorPreview.filterMode = FilterMode.Bilinear;
            _editorPreview.autoGenerateMips = false;
            _editorPreview.hideFlags = HideFlags.HideAndDontSave;
            _editorPreview.wrapMode = TextureWrapMode.Clamp;
            _editorPreview.Create();

            if(editorCamera != null)
            {
                editorCamera.targetTexture = _editorPreview;
            }
        ...
        void RemoveRenderTexture()
        {
            ...
            //Debug.Log("Remove Render Texture");
        }
		...
		void SelectedCorrectObject()
		{
            ...
            if (Event.current.type == EventType.Repaint)
            {
                if (viewAlpha)
                {
                    EditorGUI.DrawTextureAlpha(textureRect, _editorPreview);
                } else {
                    EditorGUI.DrawTextureTransparent(textureRect, _editorPreview);
                }
            }
