...
Shader "SVG Importer/UI/Default" {
	...
	SubShader
	{
		...
		Pass
		{
			...
			vertdata vert(appdata ad)
			{
			    ...
			    o.uv1.x = fmod(imageIndex, _Params.x) / _Params.x + texelOffset.x;
			    o.uv1.y = (floor((imageIndex * _Params.w) / _Params.x)) / _Params.y + texelOffset.y;
			...
			half4 frag(vertdata i) : COLOR
			{
				...
				if (_UseClipRect)
					output *= UnityGet2DClipping(i.worldPosition.xy, _ClipRect);
